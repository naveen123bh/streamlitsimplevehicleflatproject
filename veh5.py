...
def normalize_vehicle_input(vehicle_number):
    ...
    text = text.replace("O", "0")  # convert letter O to zero
    return text.strip()
...
vehicle_flat_pairs = {
    ...
    # Added manually from pasted data:
...
# ===== Normalize dictionary keys like user input =====
vehicle_flat_pairs = {normalize_vehicle_input(k): v.strip().upper() for k, v in vehicle_flat_pairs.items()}
...
if st.button("रिज़ल्ट देखें", key="lookup_button"):
    ...
    input_norm_flat = normalize_flat_input
